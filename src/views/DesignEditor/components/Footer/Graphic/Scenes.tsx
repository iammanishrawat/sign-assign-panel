...
import useContextMenuTimelineRequest from "~/hooks/useContextMenuTimelineRequest"
import SceneContextMenu from "./SceneContextMenu"
import { saveDesignToStorage, loadDesignFromStorage } from "~/helpers/banner-saved"
...
const Scenes = () => {
  ...
  const handleSave = React.useCallback(async () => {
    if (editor && frame) {
      const designJson = editor.scene.exportToJSON()
      const preview = await editor.renderer.render(designJson) as string
      saveDesignToStorage(designJson, preview, frame)
    }
  }, [editor, frame])
  ...
  React.useEffect(() => {
    if (editor) {
      ...
      editor.on("history:changed", saveWatcher)
      return () => editor.off("history:changed", saveWatcher)
    }
  }, [editor, handleSave])

  // ✅ Restore saved design from localStorage
  React.useEffect(() => {
    const restoreDesign = async () => {
      const saved = loadDesignFromStorage()
      if (editor && saved?.design && saved.preview) {
        await editor.scene.importFromJSON(saved.design)
        setCurrentScene({ ...saved.design, preview: saved.preview.url })
        setScenes([{ ...saved.design, preview: saved.preview.url }])
        setCurrentPreview(saved.preview.url)
        return
      }
  
      // fallback
      const defaultTemplate = getDefaultTemplate({ width: 1200, height: 1200 })
      await editor.scene.importFromJSON(defaultTemplate)
      const preview = await editor.renderer.render(defaultTemplate) as string
      setCurrentScene({ ...defaultTemplate, preview })
      setScenes([{ ...defaultTemplate, preview }])
